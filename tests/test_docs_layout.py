...
def test_docs_layout_renders():
    ...
    layout = DocsLayout(
        ...
        breadcrumb_items=[{"label": "Docs", "href": "/docs"}, {"label": "Test", "active": True}],
    )
    html = str(layout)
...
def test_docs_layout_without_sidebar():
    ...
    layout = DocsLayout(H1("No Sidebar"), show_sidebar=False)
    html = str(layout)
...
def test_docs_layout_with_custom_nav():
    ...
    custom_nav = [{"href": "/custom", "label": "Custom", "active": True}, {"href": "/other", "label": "Other"}]

    layout = DocsLayout(H1("Custom Nav"), nav_items=custom_nav)
    html = str(layout)
...
def test_component_customization():
    ...
    # Custom header
    header = DocsHeader(logo_text="Custom Logo", github_stars="100k", show_search=False)
    html = str(header)
    ...
    # Custom footer
    footer = DocsFooter(attribution="Built by Custom Team", hosting_info="Hosted on Custom Platform")
    html = str(footer)
