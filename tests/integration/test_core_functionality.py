        ...
        form = Form(
            Div(
                Input(
                    type="text",
                    ds_bind="user.name",
                    ds_on_input_300ms="validateName()",
            ...
            Div(
                Input(
                    type="email",
                    ds_bind="user.email",
                    ds_on_blur="validateEmail()",
                    placeholder="Email"
                ),
                Div(
                    ds_text="errors.email",
                    ds_show="errors.email",
                    style="color: red"
                )
            ),
            Button(
            ...
            Main(
                Section(
                    Article(
                        ...
                        P("Second paragraph"),
                        Div(
                            Button("Action", ds_on_click="doAction()"),
                            Span("Status", ds_text="status")
                        )
                    )
                )
            ),
        ...
        def UserCard(name, email, avatar_url):
            return Div(
                ...
                Div(
                    ...
                    Button(
                        "Follow",
                        ds_on_click=f"followUser('{email}')",
                        ds_class_active=f"isFollowing('{email}')"
                    ),
                    cls="user-info"
                ),
                cls="user-card",
...
class TestComplexFormScenarios:
    ...
    def test_dynamic_form_validation(self):
        ...
        async def validate_dynamic_form(request):
            ...
            if errors:
                return JSONResponse(
                    {"success": False, "errors": errors, "values": values},
                    status_code=422
                )
    ...
    def test_form_array_field_processing(self):
        ...
        response = client.post("/array-form",
