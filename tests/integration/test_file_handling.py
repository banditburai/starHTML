...
class TestFileUpload:
    ...
    def test_file_upload_type_validation(self):
        ...
        response = client.post(
            "/upload-validated",
            files={"file": ("test.exe", io.BytesIO(b"content"), "application/x-executable")}
        )
...
class TestFileSecurityAndValidation:
    ...
    def test_filename_sanitization(self):
        ...
        response = client.post(
            ...
            files={"file": ("normal.txt", io.BytesIO(b"content"), "text/plain")}
        )
        assert response.status_code == 200
        data = response.json()
        ...
        response = client.post(
            "/upload-filename-test",
            files={"file": ("../../../etc/passwd", io.BytesIO(b"content"), "text/plain")}
        )
        assert response.status_code == 200
        data = response.json()
    ...
    def test_file_content_validation(self):
        ...
        response = client.post(
            ...
            files={"file": ("safe.txt", io.BytesIO(b"Hello, safe content!"), "text/plain")}
        )
        assert response.status_code == 200
        data = response.json()
