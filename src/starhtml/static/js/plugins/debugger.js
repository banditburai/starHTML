import { subscribe, startObserving, stopObserving, CHIP_CATEGORIES, getEvents, formatSingleEventForExport, formatEventDetail, buildAllowedTypes, getFilteredEvents, formatAllEventsForExport, buildRowHtml } from "./debugger-capture.js";
import { clearEvents, getMorphWindow, init } from "./debugger-capture.js";
const PANEL_STYLES = `
  :host {
    position: fixed;
    bottom: 0;
    ...
    if (!this.sseToolbar || !content.contains(this.sseToolbar)) {
      ...
      this.sseEventList.addEventListener("click", (e) => {
        ...
        if (copyBtn) {
          ...
          const evts = getEvents();
          const ev = evts.find((ev2) => ev2.id === eid2);
          if (!ev) return;
        ...
        if (!wasExpanded) {
          ...
          const evts = getEvents();
          const ev = evts.find((ev2) => ev2.id === eid);
          if (ev) {
