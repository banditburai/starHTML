...
class StarHTMLDebugger extends HTMLElement {
  ...
  private setupEventListeners(): void {
    ...
    // Allow dragging from the tab-bar area too (but not from tab buttons)
  ...
  private ensureSSEStructure(): void {
    ...
    if (!this.sseToolbar || !content.contains(this.sseToolbar)) {
      ...
      this.sseEventList.addEventListener("click", (e) => {
        ...
        // Expand new row (if not collapsing)
        if (!wasExpanded) {
  ...
  private renderSSETab(): void {
    ...
    // Incremental append: if previous render is a prefix of current, only append new rows
  ...
  private buildRowHtml(ev: DebugSSEEvent): string {
  ...
  private formatEventDetail(ev: DebugSSEEvent): string {
    ...
    // Split args into data fields vs HTML content
    ...
    // For elements events: show mode + selector as a structured header
